...
void replaceFrameIndex(MachineBasicBlock::iterator II,
  ...
  if (opcode == AltairX::SPILLb || opcode == AltairX::SPILLw ||
    ...
    if(AltairX::RIReg32RegClass.contains(Reg)) {
      ...
      if(!tmpReg) {
        ...
        LLVM_DEBUG(dbgs() << "Scavenged register " << printReg(tmpReg, MBB.getParent()->getSubtarget().getRegisterInfo())
          << " for FrameReg=" << printReg(FrameReg, MBB.getParent()->getSubtarget().getRegisterInfo())
          << "+Offset=" << Offset << "\n");
  ...
  } else if (opcode == AltairX::RELOADb || opcode == AltairX::RELOADw ||
